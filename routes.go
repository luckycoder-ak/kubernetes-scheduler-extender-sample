...
func PredicateRoute(predicate predicateImpl.Predicate) httprouter.Handle {
...
func PrioritizeRoute(prioritize prioritizeImpl.Prioritize) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		checkBody(w, r)
		var buf bytes.Buffer
		body := io.TeeReader(r.Body, &buf)
		//log.Print("info: ", prioritize.Name, " ExtenderArgs = ", buf.String())
		//log.Print("Come into prioritize: ", prioritize.Name)
		var extenderArgs schedulerapi.ExtenderArgs
...
func AddPredicate(router *httprouter.Router, predicate predicateImpl.Predicate) {
	path := predicatesPrefix + "/" + predicate.Name
	router.POST(path, DebugLogging(PredicateRoute(predicate), path))
}

func AddPrioritize(router *httprouter.Router, prioritize prioritizeImpl.Prioritize) {
